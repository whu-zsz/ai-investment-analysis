...
1. 表示层
   包括 `frontend2` 中的登录页、总览页、上传页、分析页、预测页、历史页。
2. 接口层
...
前端当前使用 mock 数据驱动界面，后续将逐步替换为真实 API 调用。
...
这种设计有利于复用布局、隔离职责，并便于后续加入接口状态管理。
...
当前 `frontend2` 的界面设计特点：

- 左侧深色导航栏 + 右侧浅色内容区
- 顶部提供页面标题、接口占位状态、用户信息
- 卡片式布局承载指标、图表、提示与表格
- 登录页采用双栏设计，突出系统定位与核心功能
- 页面间视觉语言统一，便于扩展
...
| 页面 | 代码位置 |
|------|------|
| 登录页 | `frontend2/src/pages/Login.tsx` |
| 首页 | `frontend2/src/pages/Dashboard.tsx` |
| 上传页 | `frontend2/src/pages/Upload.tsx` |
| 分析页 | `frontend2/src/pages/Analysis.tsx` |
| 预测页 | `frontend2/src/pages/Prediction.tsx` |
| 历史页 | `frontend2/src/pages/History.tsx` |
| 全局样式 | `frontend2/src/index.css` |
...
3. 接口规范明确，适合前后端联调
4. 数据库结构稳定，支持后续扩展
5. UI 原型已在 `frontend2` 中落地，具备较好的展示和扩展价值
